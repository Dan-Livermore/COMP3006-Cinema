        ...
        try {
          ...
          axios
            ...
            .then((response) => {
              ...
              setLoading(false);
              axios
                .get(`http://localhost:5555/showings/${showingID}`)
                .then((response) => {
                  setShowings(response.data);
                  setLoading(false);
                  console.log("showing:", showings)
                  // Gets showingData and updates seatsRemaining -= 1
                  console.log(showings.seatsRemaining)
                  showings.seatsRemaining -= 1;
                  console.log(showings.seatsRemaining)
                  // Gets row element from rowInteger and splits on ","
                  let updateRow = ""
                  if (data.rowInteger === 0) {
                    updateRow = showings.row1.split(",");
                  } else if (rowInteger === 1) {
                    updateRow = showings.row2.split(",");
                  } else if (rowInteger === 2) {
                    updateRow = showings.row3.split(",");
                  } else {
                    updateRow = showings.row4.split(",");
                  }
                  console.log("row",updateRow)
                  // Find rowInteger[collInteger] and replace with "Occupied"
                  updateRow[colInteger] = "Occupied";
                  console.log("row updated",updateRow)
                  // Convert back to comma deliminated array
                  const newRow = updateRow.join(",");
                  // Update data object, updating seatsRemaining and the updated row
                  console.log("new row", newRow)

                  if (data.rowInteger === 0) {
                    showings = {
                      seatsRemaining: showings.seatsRemaining,
                      row1: newRow,
                    }
                  } else if (rowInteger === 1) {showings = {
                    seatsRemaining: showings.seatsRemaining,
                    row2: newRow,
                  }
                  } else if (rowInteger === 2) {showings = {
                    seatsRemaining: showings.seatsRemaining,
                    row3: newRow,
                  }
                  } else {showings = {
                  ...
                  axios
                    .put(
                      `http://localhost:5555/showings/${showingID}`,
                      showings
                    )
                    .then(() => {
                      setLoading(false);
